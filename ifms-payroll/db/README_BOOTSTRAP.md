...
3. `db/init/08_seed_data.sql` — lookup / rate / PIT seed data
4. `db/patches/003_pit_brackets_income_from.sql` — idempotent **GDT PIT** `income_from_lak` alignment (SRS §8.9; safe if seed already matches)
5. `db/init/07_triggers.sql` — audit triggers on lookup tables
6. `db/patches/002_payroll_all_view.sql` — `payroll_all` view (if file exists)
7. `db/bootstrap_seed_admin.sql` — dev user **`admin` / `password123456`**
8. Optional: `db/init/09_sample_data.sql`
