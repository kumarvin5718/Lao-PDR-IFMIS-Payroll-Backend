...
type FreeForm = {
  ...
  free_allowance_1_desc: string;
  free_allowance_2_desc: string;
  free_allowance_3_desc: string;
  free_deduction_1_desc: string;
  free_deduction_2_desc: string;
};
...
export function PayrollMonthlyPage() {
  ...
  const form = useForm<FreeForm>({
    defaultValues: {
      ...
      free_deduction_1: 0,
      free_deduction_2: 0,
      free_allowance_1_desc: "",
      free_allowance_2_desc: "",
      free_allowance_3_desc: "",
      free_deduction_1_desc: "",
      free_deduction_2_desc: "",
    },
  });
  ...
  useEffect(() => {
    if (editRow) {
      form.reset({
        ...
        free_allowance_1_desc: editRow.free_allowance_1_desc ?? "",
        free_allowance_2_desc: editRow.free_allowance_2_desc ?? "",
        free_allowance_3_desc: editRow.free_allowance_3_desc ?? "",
        free_deduction_1_desc: editRow.free_deduction_1_desc ?? "",
        free_deduction_2_desc: editRow.free_deduction_2_desc ?? "",
      });
    }
  }, [editRow, form]);
  ...
  const patchMut = useMutation({
    mutationFn: async (vals: FreeForm) => {
      if (!editRow) return;
      const res = await patchFreeFields(editRow.employee_code, editRow.payroll_month, {
        free_allowance_1: vals.free_allowance_1,
        free_allowance_2: vals.free_allowance_2,
        free_allowance_3: vals.free_allowance_3,
        free_deduction_1: vals.free_deduction_1,
        free_deduction_2: vals.free_deduction_2,
        other_allowance_1_desc: vals.free_allowance_1_desc,
        other_allowance_2_desc: vals.free_allowance_2_desc,
        other_allowance_3_desc: vals.free_allowance_3_desc,
        addl_deduction_1_desc: vals.free_deduction_1_desc,
        addl_deduction_2_desc: vals.free_deduction_2_desc,
      });
  ...
  const columns: ColumnsType<PayrollMonthlyRow> = useMemo(
    () => [
      ...
      {
        title: t("payroll.free_allowance_1"),
        key: "fa1",
        width: 112,
        align: "right",
        render: (_, row) => {
          const d = row.free_allowance_1_desc?.trim();
          const inner = (
            <div>
              <div>{Number(row.free_allowance_1).toLocaleString()}</div>
              {d ? (
                <Typography.Text type="secondary" style={{ fontSize: 11, display: "block", maxWidth: 100 }} ellipsis>
                  {d}
                </Typography.Text>
              ) : null}
            </div>
          );
          return d ? <Tooltip title={d}>{inner}</Tooltip> : inner;
        },
      },
      {
        title: t("payroll.free_allowance_2"),
        key: "fa2",
        width: 112,
        align: "right",
        render: (_, row) => {
          const d = row.free_allowance_2_desc?.trim();
          const inner = (
            <div>
              <div>{Number(row.free_allowance_2).toLocaleString()}</div>
              {d ? (
                <Typography.Text type="secondary" style={{ fontSize: 11, display: "block", maxWidth: 100 }} ellipsis>
                  {d}
                </Typography.Text>
      ...
      {
        title: t("payroll.free_allowance_3"),
        key: "fa3",
        width: 112,
        align: "right",
        render: (_, row) => {
          const d = row.free_allowance_3_desc?.trim();
          const inner = (
            <div>
              <div>{Number(row.free_allowance_3).toLocaleString()}</div>
              {d ? (
                <Typography.Text type="secondary" style={{ fontSize: 11, display: "block", maxWidth: 100 }} ellipsis>
                  {d}
                </Typography.Text>
      ...
      {
        title: t("payroll.free_deduction_1"),
        key: "fd1",
        width: 112,
        align: "right",
        render: (_, row) => {
          const d = row.free_deduction_1_desc?.trim();
          const inner = (
            <div>
              <div>{Number(row.free_deduction_1).toLocaleString()}</div>
              {d ? (
                <Typography.Text type="secondary" style={{ fontSize: 11, display: "block", maxWidth: 100 }} ellipsis>
                  {d}
                </Typography.Text>
      ...
      {
        title: t("payroll.free_deduction_2"),
        key: "fd2",
        width: 112,
        align: "right",
        render: (_, row) => {
          const d = row.free_deduction_2_desc?.trim();
          const inner = (
            <div>
              <div>{Number(row.free_deduction_2).toLocaleString()}</div>
              {d ? (
                <Typography.Text type="secondary" style={{ fontSize: 11, display: "block", maxWidth: 100 }} ellipsis>
                  {d}
                </Typography.Text>
      ...
      >
        ...
        <Space direction="vertical" style={{ width: "100%" }} size="middle">
          <div>
            ...
            />
            <Controller
              name="free_allowance_1_desc"
              control={form.control}
              render={({ field }) => (
                <Input
                  style={{ marginTop: 6 }}
                  placeholder={t("payroll.free_field_desc_placeholder_allowance")}
                  {...field}
                />
              )}
            />
          ...
          <div>
            ...
            />
            <Controller
              name="free_deduction_1_desc"
              control={form.control}
              render={({ field }) => (
                <Input
                  style={{ marginTop: 6 }}
                  placeholder={t("payroll.free_field_desc_placeholder_deduction")}
                  {...field}
                />
              )}
            />
