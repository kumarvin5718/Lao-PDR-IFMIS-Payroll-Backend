/** Screen: `EmployeeListPage` — page-level UI and mutations. */
import {
  CloseOutlined,
  EditOutlined,
  FileExcelOutlined,
      ...
      },
      {
        ...
        render: (active: boolean) => (
          <span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}>
            ...
            />
            {active ? t("employee.status_active") : t("employee.status_inactive")}
          </span>
        ),
      },
      {
        ...
        render: (_, record) => {
          ...
          const editBtn = (
            <Button
              type="text"
              icon={<EditOutlined />}
              aria-label={t("employee.aria_edit")}
              disabled={!canEdit}
          ...
          <Space>
            {!canEdit ? (
              <Tooltip title={t("employee.list_no_edit_permission")}>
                <span>{editBtn}</span>
              </Tooltip>
            ) : (
              editBtn
            ...
            >
              <Button type="text" danger icon={<CloseOutlined />} aria-label={t("employee.aria_deactivate")} />
            </Popconfirm>
          </Space>
          );
        },
