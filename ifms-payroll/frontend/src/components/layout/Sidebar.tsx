/** Layout: `Sidebar` — shell, nav, or auth guard. */
...
const MINISTRY_MASTER_ROLES = [ROLES.DEPT_OFFICER, ROLES.ADMIN] as const;
/** Audit trail list/export: backend allows ROLE_MANAGER + ROLE_ADMIN. */
const AUDIT_ROLES = [ROLES.MANAGER, ROLES.ADMIN] as const;
const REGISTRATION_ROLES = [ROLES.MANAGER, ROLES.ADMIN] as const;
const USER_MANAGEMENT_ROLES = [ROLES.ADMIN] as const;
/** Show Admin submenu if any child is visible. */
const ADMIN_SUB_ROLES = [ROLES.MANAGER, ROLES.ADMIN] as const;
const PAYROLL_PROCESSING_ROLES = [ROLES.MANAGER, ROLES.ADMIN] as const;
...
const NAV: NavItem[] = [
  ...
  },
  {
    ...
    children: [
      ...
      {
        type: "leaf",
        key: ROUTES.masterMinistry,
        labelKey: "ministryMaster.menuLabel",
        path: ROUTES.masterMinistry,
        roles: MINISTRY_MASTER_ROLES,
      },
      {
        type: "leaf",
  ...
  {
    type: "leaf",
    key: ROUTES.payrollMonthly,
    labelKey: "nav.payroll",
    path: ROUTES.payrollMonthly,
    roles: PAYROLL_PROCESSING_ROLES,
  },
  {
    type: "sub",
    key: "sub-admin",
    labelKey: "nav.menu_Admin",
    roles: ADMIN_SUB_ROLES,
    children: [
      {
        type: "leaf",
...
export function Sidebar() {
  ...
  useEffect(() => {
    setOpenKeys((prev) => {
      ...
      if (pathname.startsWith("/admin") || pathname.startsWith("/audit")) next.add("sub-admin");
