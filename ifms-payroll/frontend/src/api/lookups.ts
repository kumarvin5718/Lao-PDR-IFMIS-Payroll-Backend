/** API layer: `lookups` — typed calls to backend `/api/v1`. */
...
export interface MinistryLookup {
  ministry_key: string;
  ministry_name: string;
}

export async function getMinistries(): Promise<ApiEnvelope<MinistryLookup[]>> {
  const { data } = await apiClient.get<ApiEnvelope<MinistryLookup[]>>("/api/v1/lookups/ministries");
  return data;
}
