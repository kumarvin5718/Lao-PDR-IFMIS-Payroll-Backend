"""Report generation, downloads, and batch export jobs."""
...
from app.utils.valkey import get_report_file_owner
...
async def report_audit_log(
    ...
    """GET /reports/audit-log — ROLE_MANAGER + ROLE_ADMIN (same as finance payroll scope)."""
...
async def download_report_file(filename: str, current_user: AuthUser) -> FileResponse:
    """Download a Celery-generated file from REPORTS_DIR; only the user who queued the export may download."""
    if "/" in filename or "\\" in filename or ".." in filename:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid filename")
    safe = Path(filename).name
    if not re.match(r"^[A-Za-z0-9._-]+\.(xlsx|pdf)$", safe):
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid filename")
    owner = await get_report_file_owner(safe)
    if owner is None or owner != current_user.user_id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail={"code": "ERR_AUTH_FORBIDDEN", "message": "Not authorized to download this file"},
        )
