"""Monthly payroll run results per employee (amounts, status, lock flags)."""
...
class PayrollMonthly(Base):
    ...
    __table_args__ = (
        CheckConstraint("net_salary_lak >= 0", name="ck_payroll_net_non_negative"),
        CheckConstraint(
            "approval_status IN ('PENDING','APPROVED','LOCKED')",
            name="ck_payroll_approval_status",
        ),
    )
    ...
    payroll_month: Mapped[date] = mapped_column(Date, primary_key=True)
