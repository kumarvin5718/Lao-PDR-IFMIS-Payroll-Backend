...
2. `db/bootstrap_payroll_app_grants.sql` — `GRANT USAGE, CREATE ON SCHEMA public TO payroll_app` (required on PostgreSQL 15+)
3. `backend/scripts/create_dev_tables.py` — SQLAlchemy `create_all` (all ORM tables)
4. `db/patches/005_align_legacy_schema.sql` (if present) — adds columns / relaxes constraints on **older** DB volumes; harmless on empty DBs
5. `db/init/08_seed_data.sql` — lookup / rate / PIT seed data
6. `app.db.seeds.seed_ministry_master` + `backend/alembic/seeds/lk_location_master_154.sql` — **ministry** + **location** (district-level) reference data for master UIs
7. `db/patches/003_pit_brackets_income_from.sql` — idempotent **GDT PIT** `income_from_lak` alignment (SRS §8.9; safe if seed already matches)
8. `db/init/07_triggers.sql` — audit triggers on lookup tables
9. `db/patches/004_audit_triggers_employee_payroll.sql` — employee/payroll audit triggers (if file exists)
10. `db/patches/002_payroll_all_view.sql` — `payroll_all` view (if file exists)
11. `db/bootstrap_seed_admin.sql` — dev user **`admin` / `password123456`**
12. Optional: `db/init/09_sample_data.sql` — **organisation** + **bank** masters + **~500 demo employees** (large; not loaded by default)
